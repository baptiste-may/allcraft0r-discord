    ...
    <>
      ...
      {isLoading ? (
        ...
        <Table
          ...
          hideHeader
          defaultSelectedKeys={
            auth.isLogged ? [auth.data?.id as string] : undefined
          }
        ...
        >
          <TableHeader>
            {columns.map(({ label, id }) => (
              <TableColumn key={id}>{label}</TableColumn>
            ))}
          </TableHeader>
          <TableBody>
            {data.map(
              (
                { id, money, user: { global_name, username, avatar } },
                index,
              ) => (
                <TableRow key={id}>
                  ...
                  <TableCell>
                    <User
                      ...
                      name={global_name ?? "Utilisateur inconnu"}
                      description={"@" + (username ?? "inconnu")}
                    />
                  </TableCell>
                  <TableCell className="text-2xl font-light">{money}</TableCell>
                </TableRow>
              ),
            )}
          </TableBody>
