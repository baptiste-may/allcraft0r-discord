  ...
  try {
    for (const file of fs.readdirSync(pathname)) {
      if (file.split(".")[file.split(".").length - 1] === "ts") {
        try {
          cmds.push(
            (await import(path.join(pathname, file)))
              .default as FileCommandData,
          );
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
        } catch (err) {
          console.warn(
            chalk.yellow(
              `⚠️ Command ${path.join(pathname, file)} is not correct.`,
            ),
          );
        }
      }
...
export default async function registerCommands(client: Client): Promise<{
  ...
  const privateCommands: FileCommandData[] = await getCommands(
    path.join(commandsFolder, "private"),
  );
  ...
  for (const folder of fs.readdirSync(path.join(commandsFolder, "public"))) {
    ...
    try {
      const labelFile = fs.readFileSync(
        path.join(commandsFolder, "public", folder, "label.yaml"),
      );
      const { name, description, icon, color } = yaml.load(
        labelFile.toString("utf-8"),
      ) as {
      ...
      console.warn(
        chalk.yellow(`⚠️ Label file for category ${folder} not found.`),
      );
    }
  ...
  const helpCommand = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        ...
        .setRequired(true)
        .addChoices(
          ...Object.entries(publicCommands).map(([id, { name }]) => {
            return {
              name: name,
              value: id,
            };
  ...
  publicCommands[
    publicCommands.hasOwnProperty("utility")
      ? "utility"
      : Object.keys(publicCommands)[0]
  ].commands.push({
    data: helpCommand,
    execute: async (interaction: ChatInputCommandInteraction) => {
      const { name, description, color, commands } =
        publicCommands[interaction.options.getString("type", true)];
  ...
  Object.values(publicCommands).forEach((cat) =>
  ...
  const deployCommands = async (
    ...
    try {
      ...
      return console.error(
        chalk.yellow(`⚠️ Command deployment failed: ${error}`),
      );
    }
