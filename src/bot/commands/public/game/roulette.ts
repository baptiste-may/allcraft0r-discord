...
const NUMBERS_WITH_ZERO = [
  6, 27, 13, 36, 11, 30, 8, 23, 10, 5, 24, 16, 33, 1, 20, 14, 31, 9, 22, 18, 29,
  7, 28, 12, 35, 3, 26, 0, 32, 15, 19, 4, 21, 2, 25, 17, 34,
];
...
const createRouletteImage = async (resultNb: number) => {
  ...
  for (let i = 0; i < NUMBERS_WITH_ZERO.length; i++) {
    ...
    if (nb === resultNb) {
      ...
      const gradent = ctx.createRadialGradient(x - 2, y - 2, 1, x, y, 8);
...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Mise : ${mise} ${redstoneEmoji} ${categoriesText[category]}`,
      )
    ...
    const resColor =
      res === 0
        ? "🟢"
        : NUMBERS_WITHOUT_ZERO.indexOf(res) % 2 === 0
          ? "⚫️"
          : "🔴";
