...
async function getRandomQuestion(
  requestedType: null | "boolean" | "multiple",
): Promise<Question> {
  const res = await fetch(
    `https://opentdb.com/api.php?amount=1&difficulty=easy&encode=base64${requestedType === null ? "" : "&type=" + requestedType}`,
  );
  ...
  const {
    ...
    difficulty,
    category,
  } = (await res.json()).results[0] as Question;
  return {
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("type").setDescription("Le type de question").addChoices(
        {
          name: "Vrai / Faux",
          value: "boolean",
        },
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    const optionType = interaction.options.getString("type", false) as
      | null
      | "boolean"
      | "multiple";
    const { type, question, correct_answer, incorrect_answers } =
      await getRandomQuestion(optionType);
